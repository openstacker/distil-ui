...
from openstack_dashboard.dashboards.management import api
from openstack_dashboard.dashboards.management.api import BILLITEM
...
class BillingTests(test.TestCase):
    ...
    def test_calculate_end_date(self):
        ...
        end = api._calculate_end_date(start)
    ...
    def test_get_month_cost(self):
        ...
        distilclient.get_rated([self.request.user.tenant_id],
                                '2011-02-01T00:00:00',
                                '2011-03-01T00:00:00').AndReturn(result)
        self.mocker.ReplayAll()

        cost = [()]
        api._get_month_cost(distilclient,
    ...
    def test_calculate_history_date(self):
        ...
        start = api._calculate_start_date(datetime.date(2015, 2, 17))
        end = api._calculate_end_date(start)
    ...
    def test_get_cost(self):
        ...
        start = api._calculate_start_date(datetime.date.today())
        end = api._calculate_end_date(start)
        final_end = datetime.datetime(today.year, today.month + 1, 1)
        ...
        history_cost = api.get_cost(self.request,
    ...
    def test_apply_discount(self):
        ...
        cost = (47.54,
                [BILLITEM(id=1, resource='Compute', count=9, cost=31.76),
                 BILLITEM(id=2, resource=u'Network', count=3, cost=1.5),
                 BILLITEM(id=3, resource=u'Image', count=35, cost=3.82),
                 BILLITEM(id=4, resource=u'Router', count=2, cost=0.96),
                 BILLITEM(id=5, resource=u'Floating IP', count=21, cost=3.57),
                 BILLITEM(id=6, resource='Block Storage', count=22, cost=6.08)
                 ], [])
        ...
        cost_after_discount = api._apply_discount(cost, start_str, end_str,
                                                  prices)
        ...
        self.assertIn(BILLITEM(id=2, resource=u'Network', count=3, cost=1.05),
                      cost_after_discount[1])
        self.assertIn(BILLITEM(id=4, resource=u'Router', count=2, cost=0.48),
                      cost_after_discount[1])
    ...
    def test_get_month_cost_with_cache(self):
        ...
        self.assertIn(key, api.CACHE)
        self.assertEqual(api.CACHE[key][0], 5.05)
